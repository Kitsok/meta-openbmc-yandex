...
static int ftgmac100_setup_mdio(struct net_device *netdev)
{
	...
	if (child) {
		...
		printk("STARAT\n");
		err = of_mdiobus_register(priv->mii_bus, child);
...
static int ftgmac100_probe(struct platform_device *pdev)
{
	...
	/* Commit ecae8e8884e729b944e98fe22a6205e934d32e27 11.08.2017 */
	if (priv->use_ncsi)
	...
	if (np && of_get_property(np, "no-hw-checksum", NULL))
		netdev->hw_features &= ~(NETIF_F_HW_CSUM | NETIF_F_RXCSUM); // NETIF_F_IP_CSUM
